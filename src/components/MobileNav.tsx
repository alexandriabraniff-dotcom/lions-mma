...
export default function MobileNav({ isHome = false }: { isHome?: boolean }) {
  ...
  // Mount flag so the portal only renders on the client (not during SSR)
  ...
  // The full-screen overlay is rendered via a Portal into document.body so it
  // escapes any ancestor stacking context or containing block (e.g. the nav's
  // CSS transform which would otherwise constrain fixed positioning).
  const overlay = (
    <div
      className="fixed inset-0 bg-void flex flex-col"
      style={{
        zIndex: 200,
    ...
    >
      {/* Header row */}
      <div className="flex items-center justify-between px-6 h-16 border-b border-iron shrink-0">
        <a
          href="/"
          onClick={close}
      ...
      {/* Bottom CTA */}
      <div className="px-6 py-8 border-t border-iron flex flex-col gap-4 shrink-0">
        <a
          href="/free-trial"
          onClick={close}
  ...
  return (
    <>
      ...
      <button
        ...
        className="relative flex flex-col justify-center items-center w-11 h-11 gap-[5px]"
        style={{ zIndex: 201 }}
      >
        <span
      ...
      {/* Overlay rendered into document.body via portal */}
